...
export function ArchiveArticleView({ article, region, teaser, prev, next, basePath = '/app/archive' }: { article: A; region: { key: string; ja: string; en: string; emoji: string }; teaser: boolean; prev: N | null; next: N | null; basePath?: string }) {
  ...
  const NLink = ({ p, dir }: { p: N | null; dir: 'prev' | 'next' }) => {
    ...
    return (
      <Link href={`${basePath}/post/${p.id}`} className={`flex-1 min-w-0 rounded-xl border p-3 transition-colors ${light ? 'border-zinc-200 hover:border-zinc-400' : 'border-zinc-700/60 hover:border-zinc-500'} ${dir === 'next' ? 'text-right' : ''}`}>
    ...
    <div className={light ? 'bg-white min-h-screen' : ''}>
      <div className="px-4 md:px-8 pt-4 md:pt-8 pb-28 max-w-[680px] mx-auto">
        <div className="flex items-center justify-between gap-3 mb-6">
          <Link href={`${basePath}/${region.key}`} className={`text-sm ${muted} hover:underline`}>← {region.emoji} {ja ? region.ja : region.en}</Link>
        ...
        <article>
          ...
          {teaser && <Paywall light={light} ja={ja} returnTo={`${basePath}/post/${article.id}`} />}
